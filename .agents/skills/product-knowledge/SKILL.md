---
name: product-knowledge
description: Maintain and retrieve Stream Bro product truth with a tiny core overview and detailed on-demand references. Use before product decisions and after changes to features, behavior, architecture, assets, constraints, or roadmap.
---

# Product Knowledge

Stream Bro is a local-first streaming toolkit for OBS. It turns webcam expression and head movement into a layered 2D avatar and will grow into a home for more streaming tools. The web app lives in `apps/web` and uses Next.js.

## Knowledge rule

- Always update this overview when the product purpose, audience, core stack, or main feature changes.
- Store detailed and changing facts in references, not here.
- Update only affected references.
- Record facts, decisions, open questions, and user TODOs. Remove stale facts.
- Keep each fact in one place.

## Read as needed

- Avatar behavior, asset count, filenames, tracking, and acceptance: [avatar.md](references/avatar.md)
- Current roadmap, open questions, and user tasks: [roadmap.md](references/roadmap.md)

